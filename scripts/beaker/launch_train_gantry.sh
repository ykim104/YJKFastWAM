...
GANTRY_ARGS=(
  ...
  --install " (command -v apt-get >/dev/null && apt-get update -qq && apt-get install -y -qq python3.10-dev build-essential) || true; uv pip install nvidia-cuda-nvcc-cu12; unset CUDA_HOME; python -c \"import nvidia.cuda_nvcc, accelerate, torch; import fastwam; import os; from pathlib import Path; import nvidia.cuda_nvcc as m; p=Path(m.__file__).resolve().parent; assert (p/'bin'/'nvcc').is_file(), p; print('nvcc', p/'bin'/'nvcc')\""
  --name "fastwam-${TASK}"
